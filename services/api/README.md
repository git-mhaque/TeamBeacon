...
- `GET /api/team/dashboard?flowWeeks=4&recentLimit=5&timezone=Australia%2FMelbourne`
  - Add `cycleTimeStatusMode=custom` with repeated `cycleTimeStatus=<normalized-status-key>` parameters to keep the sprint cycle-time summary aligned with the saved Team Insights workflow-status selection.
