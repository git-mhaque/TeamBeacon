...
- **Initiative Deep Dive:** default to all groups with checkbox-based multi-group selection, cascade their combined scope into an all-or-multi-epic selector, compare new and completed cards over a persisted preset-or-custom trend range (12 weeks by default), use independent 1/2/4/12-week tiles to drive a horizontally contained activity table, and expose Current WIP as a distinct all-age snapshot.
